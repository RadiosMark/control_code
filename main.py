    ...
    def automatico(self):  # instrucciones para seguir la mira (no se pudo implementar completo)
        if self.llego_vertical == False or self.llego_horizontal == False:
            ...
            if (800-self.pos[1]) > (self.mira.pos[1]) and self.vertical == 'Abajo':
                ...
                #    self.u2 -= 30
            if self.theta > arctan((self.pos[1]-self.mira.pos[1])/(+self.mira.pos[0]-self.pos[0])) and \
            self.horizontal == 'Derecha' and self.vertical == 'Arriba' and self.llego_horizontal == False:
                self.u2 -= 10
            if self.theta < arctan((self.pos[1]-self.mira.pos[1])/(+self.mira.pos[0]-self.pos[0])) and \
            self.horizontal == 'Derecha' and self.vertical == 'Arriba' and self.llego_horizontal == False:
                self.u2 = 0
            if self.theta < arctan((-self.mira.pos[0]+self.pos[0])/(self.pos[1]-self.mira.pos[1])) + pi/2 and \
            self.horizontal == 'Izquierda'and self.vertical == 'Arriba'  and self.llego_horizontal == False:
                self.u2 += 10
            if self.theta > arctan((-self.mira.pos[0]+self.pos[0])/(self.pos[1]-self.mira.pos[1])) + pi/2  and \
            self.horizontal == 'Izquierda' and self.vertical == 'Arriba' and self.llego_horizontal == False:
                self.u2 = 0
